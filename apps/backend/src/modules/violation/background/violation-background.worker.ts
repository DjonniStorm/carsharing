...
import { IJobQueueToken, type IJobQueue } from 'src/shared/background/job-queue.interface';
...
export class ViolationBackgroundWorker implements OnModuleInit {
  ...
  private async tick(): Promise<void> {
    ...
    try {
      if (job.name === ViolationJobName.RentalMovementZoneCheck) {
        await this.handleRentalMovement(job.payload as RentalMovementZoneCheckJob);
        return;
      }
  ...
  private async handleRentalMovement(input: RentalMovementZoneCheckJob): Promise<void> {
