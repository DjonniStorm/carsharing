...
const TripViewTripDetailsStaticBody = memo(function TripViewTripDetailsStaticBody({
  trip,
  t,
  userById,
}: StaticProps) {
...
function TripViewTripLiveMetricsBody({ trip, t }: LiveProps) {
  const { i18n } = useTranslation();
  const tk = (key: LangKey) => t(key);
  const locale = i18n.language;
...
export function TripViewTripDetailsSection({ trip, liveTrip, t, userById }: Props) {
  const tk = (key: LangKey) => t(key);
  ...
  return (
    <Paper p="md" radius="md" withBorder>
      <Stack gap="sm">
        ...
        <Divider />
        <TripViewTripDetailsStaticBody trip={trip} t={t} userById={userById} />
        <TripViewTripLiveMetricsBody trip={liveTrip} t={t} />
      </Stack>
    </Paper>
  );
}
