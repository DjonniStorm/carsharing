...
export class BaseApiClient {
  ...
  async requestJson<T>(path: string, init: JsonRequestOptions = {}): Promise<T> {
    ...
    if (!res.ok) {
      throw new HttpApiError(
        pickMessage(
          data,
          translate(LANG_KEYS.api.requestFailedWithStatus, { status: res.status }),
        ),
        res.status,
      )
    }
  ...
  postJson<T>(path: string, json: unknown, init?: JsonRequestOptions): Promise<T> {
    return this.requestJson<T>(path, { ...init, method: 'POST', json })
  }

  patchJson<T>(path: string, json: unknown, init?: JsonRequestOptions): Promise<T> {
