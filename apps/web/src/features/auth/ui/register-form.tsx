...
import { Anchor, Button, PasswordInput, Radio, Stack, Text, TextInput, Title } from '@mantine/core'
...
export const RegisterFormView = () => {
  ...
  const handleSubmit = useAction(async (values: RegisterFormOutput) => {
    try {
      ...
      const msg =
        err instanceof Error ? err.message : t(LANG_KEYS.auth.notifyRegisterErrorFallback)
    ...
    <form onSubmit={form.onSubmit(handleSubmit)}>
      <Stack gap="md">
        ...
        <Text size="sm" c="dimmed">
          {t(LANG_KEYS.auth.registerHintBefore)} <code>POST /auth/register</code>.{' '}
        ...
        {OPEN_MANAGER_SELF_REGISTER ? (
          ...
          >
            <Stack gap="xs" mt="xs">
              <Radio value={UserRole.MANAGER} label={t(LANG_KEYS.auth.roleManager)} />
              <Radio value={UserRole.DRIVER} label={t(LANG_KEYS.auth.roleDriver)} />
            </Stack>
          </Radio.Group>
        ) : null}
